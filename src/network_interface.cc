...
{
  if (frame.header.dst != ethernet_address_ && frame.header.dst != ETHERNET_BROADCAST) return; //how to do broadcast addr
...
void NetworkInterface::tick( const size_t ms_since_last_tick )
{
  ...
  for (auto i = arp_requests_waiting_.begin(); i != arp_requests_waiting_.end();) {
    if (current_time_ - i->second > 5000) { //expire after 30s
      if (datagrams_waiting_.contains(i->first)) datagrams_waiting_.erase(i->first); //erase corresponding datagram queue for this IP as well
