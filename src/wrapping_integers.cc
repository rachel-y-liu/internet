...
uint64_t Wrap32::unwrap( Wrap32 zero_point, uint64_t checkpoint ) const
{
  ...
  if ( diffhigh < diffmed )
    unwrapped = high; // if option2 is better, choose option2
  if ( med >= ( 1ULL << 32 ) && difflow < diffmed && difflow < diffhigh )
    unwrapped = low; // if option1 exists and is better, choose option1
