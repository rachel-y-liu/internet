...
void TCPSender::push( const TransmitFunction& transmit )
{
  ...
  while ( !is_last_msg ) {
    ...
    else if ( sequence_numbers_in_flight() < window_size_ )
      remaining_window_size = window_size_ - sequence_numbers_in_flight();
    ...
    uint64_t num_bytes_to_read = min( {
      input_.reader().bytes_buffered(), remaining_window_size
...
void TCPSender::receive( const TCPReceiverMessage& msg )
{
  ...
  if ( msg_abs_ackno > abs_ackno_ + sequence_numbers_in_flight() )
    return;
  ...
  if ( msg_abs_ackno > abs_ackno_ ) { // if new data was acked (yay!)
    ...
    while ( true && sequence_numbers_in_flight() > 0 ) {
      ...
      uint64_t outstanding_msg_start
        = outstanding_msg.seqno.unwrap( isn_, input_.reader().bytes_popped() ); // abs seqno
