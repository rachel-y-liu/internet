...
------ HOW I KEPT TRACK OF WINDOW SIZE ------
...
----- TRACKING THE SEQNO AND ACKNO -----
I also tracked abs_seqno(the next seqno that has not yet been popped from the bytestream) and abs_ackno (the last seqno that has been acknowledged). I could also have tracked stream index, but it would've required an additional state variable to track SYN and FIN flags, which would've added unnecessary complexity and lines of code.

-------- USING MY TCP! --------
My TCP successfully interoperated with tcp_native! The connection gets establishe dnad each peer can type at each other. Both programs quit cleanly after both streams have hit 'ctrl+D'. The program (lets call it Peer A) must wait a little if it is the first to close. After Peer A sends the first FIN, it must wait for Peer B to send a FIN, then Peer A sends a final ACK. It must wait to see if Peer B retransmits its FIN, in which case it knows that its final ACK was not received and it must retransmit the final ACK. In this case, if Peer A had exited, there would be no acknowledgement of Peer B's final FIN. This is the reason for the wait. 

Output after running 'ls -l /tmp/big.txt' : '-rw-rw-r-- 1 cs144 cs144 1000000 Oct 22 21:35 /tmp/big-received.txt'
Size of file = 1000000 bytes
Output after running 'sha256sum /tmp/big-received.txt' : 7a18b716189ae527fb1a7c0e5194336327dbc669be55f7d07b57905d7a1d9895
